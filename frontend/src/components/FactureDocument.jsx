/**
 * Document facture A4 — une seule page (aperçu = PDF).
 * Somme en lettres sous le total ; description de formation affichée.
 */
...
export default function FactureDocument({
  ...
  return (
    ...
    >
      <div className="h-[2px] w-full shrink-0" style={{ background: primary }} />

      <header className="a4-row justify-between gap-[5mm] px-[14mm] pb-[3mm] pt-[9mm]">
        ...
        <div className="w-[46mm] shrink-0 text-right">
          <p className="text-[10.5px] font-bold uppercase tracking-[0.08em] text-slate-900">
            {titreTypeDocument(facture?.type_document, { uppercase: false })}
          </p>
          <div className="mt-1.5 space-y-0.5 border-t border-slate-300 pt-1.5 text-[9.5px] text-slate-700">
            <p>
      ...
      <div className="mx-[14mm] border-b border-slate-300" />

      <section className="a4-grid-2 gap-[5mm] px-[14mm] py-[3mm]">
        <div>
          <p className="text-[8.5px] font-semibold uppercase tracking-[0.12em] text-slate-500">Bénéficiaire</p>
          ...
          {definitive && etudiant.nationalite && (
            <p className="text-[9.5px] text-slate-600">Nationalité : {etudiant.nationalite}</p>
          )}
        ...
        <div>
          <p className="text-[8.5px] font-semibold uppercase tracking-[0.12em] text-slate-500">Formation</p>
          ...
          {description ? (
            <p className="mt-1 max-h-[18mm] overflow-hidden text-[9px] leading-snug text-slate-600">
              {description}
            </p>
          ) : null}
        </div>
        ...
        <table className="text-[10.5px]">
          <thead>
            <tr className="border-y border-slate-800">
              <th className="py-1.5 text-left text-[8.5px] font-semibold uppercase tracking-wide text-slate-800">
                Désignation
              </th>
              <th className="w-[38mm] py-1.5 text-right text-[8.5px] font-semibold uppercase tracking-wide text-slate-800">
                Montant (FCFA)
              </th>
            </tr>
          </thead>
          ...
          <tfoot>
            <tr>
              <td className="border-t-2 border-slate-800 py-2 text-[11px] font-bold text-slate-900">
                Montant total à payer
              </td>
              <td
                className="border-t-2 border-slate-800 py-2 text-right text-[12px] font-bold tabular-nums"
                style={{ color: primary }}
              >
            ...
            <tr>
              <td colSpan={2} className="pb-1 pt-1.5 text-[10.5px] font-bold leading-snug text-slate-900">
      ...
      {payLines.length > 0 && (
        <section className="px-[14mm] pb-[2.5mm]">
          <p className="mb-1 text-[8.5px] font-semibold uppercase tracking-[0.12em] text-slate-500">
            Coordonnées de paiement
          </p>
          <div className="border border-slate-200 bg-slate-50 px-2.5 py-1.5 text-[9.5px] text-slate-700">
            {payLines.map((l) => (
      ...
      <footer className="border-t border-slate-300 px-[14mm] py-1.5 text-center text-[8px] text-slate-500">
        {[nomEtab, etab?.email_contact, etab?.telephone].filter(Boolean).join(' · ')}
      </footer>
      <div className="h-[1.5px] w-full shrink-0" style={{ background: secondary }} />
    </article>
  )
}
