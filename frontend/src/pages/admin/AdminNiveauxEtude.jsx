...
export default function AdminNiveauxEtude({ etabOnly = false }) {
  const { user } = useAuth()
  const isEtabAdmin = etabOnly || user?.role === 'admin_etablissement'
  ...
  const openEdit = (n) => {
    if (isEtabAdmin && !n.etablissement_id) {
      toast.error('Les niveaux globaux plateforme ne sont pas modifiables ici.')
      return
    }
    setEditing(n)
    setForm({
  ...
  const toggleActif = async (n) => {
    if (isEtabAdmin && !n.etablissement_id) return
  ...
  const canManage = (n) => !isEtabAdmin || !!n.etablissement_id

  return (
    <div className="mx-auto max-w-4xl space-y-4 px-4 py-6 min-w-0">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          {isEtabAdmin && (
            <Link to="/mon-etablissement" className="text-sm text-gray-400 hover:text-blue-700">
              ← Mon établissement
            </Link>
          )}
          <h1 className="text-2xl font-black text-gray-900">
            {isEtabAdmin ? 'Niveaux d’étude — mon établissement' : 'Niveaux d’étude'}
          </h1>
          <p className="text-sm text-gray-500">
            {isEtabAdmin
              ? 'Créez et gérez les niveaux propres à votre établissement. Les niveaux globaux plateforme sont affichés en lecture seule.'
                ...
                {niveaux.map((n) => (
                  <tr key={n.id} className="hover:bg-gray-50/80">
                    ...
                    <td className="px-4 py-3">
                      <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${n.etablissement_id ? 'bg-blue-100 text-blue-800' : 'bg-slate-100 text-slate-600'}`}>
                        {n.etablissement_id ? 'Établissement' : 'Global'}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <span
