...
export default function IdentiteBeneficiaireProforma({
  ...
  /** Adresse physique obligatoire (demande de facture). */
  addressRequired = false,
  /** Afficher année académique (libre, ex. 2025-2026). */
  showAnneeAcademique = false,
  anneeAcademiqueRequired = false,
}) {
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2">
      ...
      <div className="sm:col-span-2">
        <label className="mb-1 block text-sm font-semibold">
          Adresse physique {addressRequired && <span className="text-red-500">*</span>}
        </label>
        <textarea
          ...
          placeholder="Adresse postale complète du bénéficiaire"
        />
      </div>
      {showAnneeAcademique && (
        <div className="sm:col-span-2">
          <label className="mb-1 block text-sm font-semibold">
            Année académique {anneeAcademiqueRequired && <span className="text-red-500">*</span>}
          </label>
          <input
            className="input-field"
            value={form.annee_academique || ''}
            onChange={up('annee_academique')}
            required={anneeAcademiqueRequired}
            placeholder="Ex. 2025-2026"
            list="annees-academiques-suggest"
          />
          <datalist id="annees-academiques-suggest">
            {(() => {
              const y = new Date().getFullYear()
              return [0, 1, 2].map((i) => {
                const a = y + i
                return <option key={a} value={`${a}-${a + 1}`} />
              })
            })()}
          </datalist>
          <p className="mt-1 text-xs text-slate-500">Saisie libre (ex. 2025-2026).</p>
        </div>
      )}
