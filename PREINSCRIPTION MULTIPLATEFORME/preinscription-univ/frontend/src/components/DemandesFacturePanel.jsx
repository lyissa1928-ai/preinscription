...
function sourceLabel(d) {
  if (d.source === 'public_distant') return { text: 'Visiteur', cls: 'bg-violet-100 text-violet-800' }
  if (d.etudiant_id) return { text: 'Compte candidat', cls: 'bg-sky-100 text-sky-800' }
  return { text: 'Demande', cls: 'bg-slate-100 text-slate-700' }
}

function candidatLabel(d) {
  if (d.source === 'public_distant') return d.email || '—'
  const name = `${d.prenom || ''} ${d.nom || ''}`.trim()
  return name || d.email || '—'
}

/**
 * Liste des demandes de facture à traiter (+ acceptées).
 * @param {string} apiBase — '/api/responsable' ou '/api/admin'
 */
export default function DemandesFacturePanel({ apiBase = '/api/responsable', embedded = false }) {
  const { user } = useAuth()
  ...
  const load = (p = page) => {
    setLoading(true)
    axios
      .get(`${apiBase}/demandes-proforma?page=${p}&limit=20`)
      .then(({ data }) => {
  ...
  useEffect(() => {
    load(page)
  }, [page, apiBase]) // eslint-disable-line react-hooks/exhaustive-deps

  const visible = useMemo(() => {
    const list = Array.isArray(demandes) ? demandes : []
    if (onglet === 'a_traiter') return list.filter((d) => isPending(d.statut))
    if (onglet === 'acceptations') return list.filter((d) => d.statut === 'acceptee')
    return list
  }, [demandes, onglet])

  const marquerVue = (id) => {
    axios
      .put(`${apiBase}/demandes-proforma/${id}/statut`, { statut: 'vue' })
  ...
  const confirmerDecision = async () => {
    ...
    setSaving(true)
    try {
      await axios.put(`${apiBase}/demandes-proforma/${demande.id}/decision`, {
        decision: type === 'accepter' ? 'accepter' : 'refuser',
        motif_refus: type === 'refuser' ? motifRefus.trim() : undefined,
        avec_cachet: type === 'accepter' ? avecCachet : undefined,
      })
  ...
  const sendFactureEmail = async (demandeId) => {
    setEmailSending(demandeId)
    try {
      const { data } = await axios.post(`${apiBase}/demandes-proforma/${demandeId}/envoyer-email`)
  ...
  const justifLinks = (d) => {
    const j = d.justificatifs || {}
    return [
      j.identite && { key: 'identite', label: 'Identité', href: justifUrl(j.identite) },
      j.diplome && { key: 'diplome', label: 'Diplôme', href: justifUrl(j.diplome) },
      j.releve && { key: 'releve', label: 'Relevé', href: justifUrl(j.releve) },
      j.formation && { key: 'formation', label: 'Formation', href: justifUrl(j.formation) },
    ].filter(Boolean)
  }

  return (
    <div className={embedded ? '' : 'mx-auto w-full max-w-6xl px-4 py-5'}>
      <div className="mb-4 flex flex-wrap gap-1 rounded-xl border border-slate-200 bg-white p-1 text-sm font-semibold">
        {[
          ...
          { id: 'acceptations', label: 'Acceptées' },
        ].map((t) => (
          <button
            key={t.id}
            type="button"
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-dashed border-slate-200 bg-white py-12 text-center text-sm text-slate-400">
          {onglet === 'a_traiter' ? 'Aucune demande de facture en attente.' : 'Aucune facture acceptée pour le moment.'}
        </div>
      ) : (
        <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
          <div className="max-h-[min(62vh,560px)] overflow-auto overflow-x-auto">
            <table className="w-full min-w-[860px] text-left text-sm">
              <thead className="sticky top-0 z-10 bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
                <tr>
                  <th className="px-3 py-2.5 font-semibold">Demandeur</th>
                ...
                {visible.map((d) => (
                  ...
                  >
                    <td className="px-3 py-2.5 min-w-[10rem]">
                      <div className="font-semibold text-slate-900">{candidatLabel(d)}</div>
                      <div className="text-xs text-slate-500 break-all">{d.email || '—'}</div>
                      {d.adresse && (
                        <div className="text-[11px] text-slate-500 mt-0.5 break-words" title={d.adresse}>
                          {d.adresse}
                        </div>
                      )}
                      {d.annee_academique && (
                        <div className="text-[11px] font-medium text-indigo-700 mt-0.5">
                          Année {d.annee_academique}
                        </div>
                      )}
                    ...
                    <td className="px-3 py-2.5">
                      {(() => {
                        ...
                        return (
                          <span className={`inline-block rounded-full px-2 py-0.5 text-[10px] font-bold ${src.cls}`}>
                            {src.text}
                          </span>
                        )
                      })()}
                    </td>
                    <td className="max-w-[220px] px-3 py-2.5 text-slate-700" title={d.formation_titre}>
                      <span className="line-clamp-2">{d.formation_titre}</span>
                    </td>
                      ...
                      <div className="flex flex-wrap gap-1">
                        ...
                        {d.statut === 'acceptee' && d.reference && (
                          <>
                            ...
                            <button
                              type="button"
                              disabled={emailSending === d.id || !d.facture?.numero}
                              onClick={() => sendFactureEmail(d.id)}
                              className="rounded-md border border-violet-200 px-2 py-1 text-xs font-bold text-violet-800 hover:bg-violet-50 disabled:opacity-40"
                            >
                              {emailSending === d.id ? 'Envoi…' : 'Envoyer par e-mail'}
                            </button>
                          </>
                        )}
            ...
            {modal.type === 'accepter' && (
              <>
                <p className="mt-3 text-sm text-emerald-800">
                  La facture sera générée. Choisissez si le cachet apparaît sur le document.
                </p>
                <div className="mt-3 space-y-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm">
                  <label className="flex cursor-pointer items-center gap-2">
                    <input type="radio" name="cachet_panel" checked={avecCachet} onChange={() => setAvecCachet(true)} />
                    Facture avec cachet
                  </label>
                  <label className="flex cursor-pointer items-center gap-2">
                    <input type="radio" name="cachet_panel" checked={!avecCachet} onChange={() => setAvecCachet(false)} />
                    Facture sans cachet
                  </label>
                </div>
              </>
