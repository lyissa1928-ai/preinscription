    ...
    <div>
      ...
      <div className="mt-2 flex flex-wrap items-center gap-3">
        ...
        <button
          type="button"
          onClick={() =>
            downloadFile(`/courses/export/me-pdf?anneeUniv=${anneeUniv}`, `Mon_EDT_${anneeUniv}.pdf`).catch(() => {})
          }
          className="px-3 py-2 rounded bg-rose-700 text-white text-sm hover:bg-rose-800"
        >
                  ...
                  return (
                    <td key={j} rowSpan={span} className="p-2 align-top border-l">
                      <div className="bg-blue-50 rounded p-2 text-xs">
                        ...
                        <div>{c.salle.nom}</div>
                        {c.salle.campus && (
                          <div className="text-slate-600 mt-0.5">📍 {c.salle.campus.nom}</div>
                        )}
                      </div>
                    </td>
                  );
